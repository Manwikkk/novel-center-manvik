...
export default function UnlockModal({ open, chapter, balance, onConfirm, onClose, busy }) {
  ...
  return (
    ...
    >
      <p className="font-serif text-[18px] text-ink-700">{chapter.title}</p>
      <dl className="mt-6 grid grid-cols-2 gap-4 text-[14px]">
        <div>
          <dt className="label-sm text-ink-400">Cost</dt>
          <dd className="mt-1 font-serif text-[20px] text-ink-900">{formatTokens(chapter.tokenPrice)} tokens</dd>
        </div>
        <div>
          <dt className="label-sm text-ink-400">Your balance</dt>
          <dd className="mt-1 font-serif text-[20px] text-ink-900">{formatTokens(balance)} tokens</dd>
        </div>
      </dl>
      {insufficient && (
