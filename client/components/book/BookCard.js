...
export default function BookCard({ book, layout = 'vertical', className, kicker }) {
  ...
  if (layout === 'horizontal') {
    return (
      <Link
        href={href}
        className={cn(
          'flex items-center gap-4 p-4 rounded-lg hover:bg-surface-container-low transition-colors cursor-pointer group border border-transparent hover:border-surface-container-high',
          className,
        )}
      >
        <img
        ...
        <div className="min-w-0">
          <h5 className="font-ui-label-lg text-ui-label-lg text-on-surface group-hover:text-primary transition-colors truncate">
            {book.title}
          </h5>
          <p className="font-ui-label-sm text-ui-label-sm text-on-surface-variant truncate">
            {book.authorName}
          </p>
        </div>
        {typeof kicker === 'string' && (
          <div className="ml-auto text-right">
            <span className="font-ui-label-sm text-ui-label-sm text-on-surface-variant">{kicker}</span>
          </div>
        )}
      </Link>
    );
  ...
  if (layout === 'scroll') {
    return (
      ...
      >
        ...
        <div>
          <h4 className="font-ui-label-lg text-ui-label-lg text-on-surface mb-1 truncate">
            {book.title}
          </h4>
          <p className="font-ui-label-sm text-ui-label-sm text-on-surface-variant mb-2 truncate">
            {book.authorName}
          </p>
          {kicker && (
            <p className="font-ui-label-sm text-ui-label-sm text-on-surface-variant/70 italic">
              {kicker}
            </p>
          )}
        </div>
  ...
  return (
    <Link href={href} className={cn('group cursor-pointer flex flex-col gap-4', className)}>
      <div className="aspect-[2/3] bg-surface-container border border-surface-variant p-1 shadow-sm overflow-hidden">
        <img
          alt={book.title}
          src={cover}
      ...
      <div>
        {book.category && (
          <p className="font-ui-label-sm text-ui-label-sm text-on-surface-variant uppercase mb-1">
            {book.category}
          </p>
        )}
        <h3 className="font-headline-md text-on-surface text-[24px] leading-tight mb-1">
          {book.title}
        </h3>
        <p className="font-reading-body text-[16px] text-on-surface-variant">
          by {book.authorName}
        </p>
      </div>
    </Link>
